  ...
  reducers: {
    addToCart: (state, action) => {
      const item = state.products.find(
        (item) => ((item.id_product === action.payload.id_product and item.id_product === action.payload.id_product)) 
      );
      if (item) {
        let subItem=state.products.find(
          (item) => ((item.id_variant === action.payload.id_variant))
        )
        if(subItem)
