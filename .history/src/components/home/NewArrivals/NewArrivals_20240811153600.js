...
const NewArrivals = ({props}) => {
  ...
  return (
    ...
    {productShop.map((item, index) => (
        <div key={index} className="hover:scale-105 transition-all duration-300 px-4 my-2">
           <Link to={`/product/${item.id_product}`}>
            <img
              className="px-4 w-full border border-b-0 rounded-t-lg hover:shadow-md"
              src={item.avt_product}
              alt="productImage"
            />
          </Link>
          <div className="product-item bg-light mb-4 border hover:shadow-md rounded-b-lg border-t-0 px-4">
