...
const NewArrivals = ({props}) => {
  ...
  return (
    ...
    {productShop.map((item, index) => (
        <div key={index} className="hover:scale-105 transition-all duration-300 border-[1px] border-t-0 rounded-b-lg mx-4">
           <Link to={`/product/${item.id_product}`}>
            <img
              className="mx-4 w-full border "
              src={item.avt_product}
              alt="productImage"
            />
          </Link>
          <div className="product-item bg-light mb-4 border px-4">
