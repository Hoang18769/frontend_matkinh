...
const ProductDetails = () => {
  ...
  return (
    ...
    >
      ...
      <Rating name="read-only" value={feedback?Math.round(feedback.reduce((a,b)=>a+b.rating,0)):0} readOnly />
    </Box>
          ...
          <div className="flex items-center" onClick={()=>{setShow(!show)}}> 
              {feedback?<>
                <h3 className="text-gray-600 text-2xl my-4 " >Đánh giá sản phẩm </h3>           <IoMdArrowDropdown  />

              </>          
:          <h3 className="text-gray-600 text-2xl my-4 " >Chưa có đánh giá nào </h3> 
}
          </div>

        {feedback?.map((item)=>(
          <div className=" md:px-6 2xl:px-0 2xl:container 2xl:mx-auto  flex justify-center items-center" style={{display: show ? 'flex' : 'none' }}>
          ...
          <div className="w-full flex justify-start items-start flex-col p-8 ">
            <div id="menu" className="md:block ">
              ...
              <div className=" w-full flex justify-between items-center flex-row space-x-2.5 ">
                ...
                <p className="px-4">Phân loại: {item.desc_color} {item.desc_size} </p>
                <Box
      sx={{
        '& > legend': { mt: 2 },
      }}
