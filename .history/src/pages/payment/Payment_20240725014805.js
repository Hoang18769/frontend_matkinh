...
const Payment = () => {
  ...
  let handleApplyDiscount=(e)=>{
    e.preventDefault();
    console.log(voucher)
    let discountCode=voucher?.find((item)=>(
      item.code===id_discount.toUpperCase()
    ));
    if(!discountCode){
    ...
    else if(discountCode.payment_limit>totalAmt){
      ...
      setDiscount(discountCode.discount)
  ...
  const submitOrder = (e) => {
    ...
    axios
      .post("https://matkinhcaolo.io.vn/api/orders", {
        ...
        note: note,
        total_order: totalAmt + shippingCharge- {discount}.discount,
        ...
        <div className=" gap-4 w-2/5">
          <div className="w-96 gap-3">
            ...
            <div className="bg-white shadow-md rounded px-4 pt-4 pb-8 mb-4 my-2 ">
              ...
              {
                voucher.find((item)=>(
                item.code===id_discount.toUpperCase()&&item.payment_limit<=totalAmt
