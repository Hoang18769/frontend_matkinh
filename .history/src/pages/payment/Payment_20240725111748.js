...
const Payment = () => {
  ...
  const [id_Discount, setId_Discount] = useState(0);
  ...
  let handleApplyDiscount=(e)=>{
    ...
    let date=new Date();
    let dateVoucher=new Date(discountCodeValid.expiration_date.toString());
    console.log(date+" and"+ dateVoucher)
    console.log(dateVoucher-date)

    //console.log(discountCodeValid.expiration_date)
    ...
    else if(dateVoucher-date>86400){
      setErrDiscount("Mã giảm giá ")
    }
        ...
        <div className=" gap-4 w-2/5">
          <div className="w-96 gap-3">
            ...
            <div className="bg-white shadow-md rounded px-4 pt-4 pb-8 mb-4 my-2 ">
              ...
              <p className="text-lg appearance-none flex items-center justify-between w-full bg-grey-lighter text-grey-darker border border-red rounded py-3 px-4 mb-3">
                Khuyến mãi
                {({apply_voucher}.apply_voucher)?<span className="font-medium tracking-wide font-titleFont">
                  - {discount} VND
                </span>
                : <span className="font-medium tracking-wide font-titleFont">
                  0 VND
              ...
              {
                voucher.find((item)=>(
                item.code===discountCode.toUpperCase()&&item.payment_limit<=totalAmt
