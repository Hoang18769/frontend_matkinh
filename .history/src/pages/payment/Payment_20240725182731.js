      ...
      .catch((error) => {
        ...
        if(error.response.request.status===400){
          error.response.data.product_variant?.map((item)=>(
            failedProduct+=item.name_product +item.id_variant+"số lượng còn lại: "+item.quantity_available+"</br>"
          ))
          swal(error.response.data.message, <span>
            
          </span>error.response.data.error +failedProduct, "failed");
