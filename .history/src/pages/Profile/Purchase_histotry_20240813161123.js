...
const Purchase_history = () => {
  ...
  return (
    <div className="w-full lg:flex sm:flex-row px-8 sm:px-0 py-8 ">
      <Profile/>

      <div className="w-4/5 sm:w-full items-center px-4 sm:px-0">
        <div className="w-full flex px-8 sm:px-0 py-4 font-bold text-lg bg-[#F5F5F3] rounded-md">
          {list.map((item) => (
            <label className="lg:w-52 sm:w-20 sm:w-20 py-4  text-center">{item}</label>
          ))}
          <label className="invisible  w-52 sm:w-20 sm:w-20 py-4 ">hidden</label>
        </div>
        <div className=" ">
          {history.toReversed().map((item) => (
            <div className=" rounded-md w-full border-b  px-8 sm:px-0 items-center ">
              <div>
                <div className=" items-center flex justify-between">
                  <div className="w-full flex justify-between ">
                    ...
                    >
                      <div className="flex py-4 rounded-md text-base items-center text-center hover:underline underline-offset-[4px] ">
                        <div className="w-52 sm:w-20 sm:w-20 px-4 sm:px-0 py-2 ">{item.id_order}</div>
                        <div className="w-52 sm:w-20 sm:w-20 px-4 sm:px-0 py-2 ">{item.date_order}</div>
                        <div className="w-52 sm:w-20 sm:w-20 px-4 sm:px-0 py-2 ">
                  ...
                  item.order_details.length === item.feedback.length ? (
                    <p class=" w-48 py-3 text-center  text-sm font-semibold rounded-lg bg-yellow-100 text-yellow-600">
                      Bạn đã đánh giá rồi
                    </p>
                  ) : item.status_order === 4 &&
                    item.order_details.length !== item.feedback.length ? (
