...
const Purchase_history = () => {
  ...
  const fetchData = async () => {
    try {
     ...
     total=history.reduce((a,b)=> {return parseInt(a)+parseInt(b.total_order),0)
     console.log(total)
              ...
              <div>
               <div className=" items-center flex justify-between">
                ...
                <div onClick={()=>handleDetail(item.id_order)} className="w-full">
                  ...
                  :(item.status_order === 4 && item.order_details.length!==item.feedback.length)?
                  <>
                  <div  onClick={()=>handleFeedback(item)}
                        className="text-center  w-52 hover:bg-[#F5F5F3]">Đánh giá</div>
                  </>:
                  (item.status_order !== 4)?
                  null:null
                  }
