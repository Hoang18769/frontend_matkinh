            ...
            <div className=" rounded-md w-full border-b  px-8 items-center ">
              <Menu>
                ...
                <div className="w-fit">
                  <MenuItems
                    anchor="bottom"
                    className=" w-fit items-center flex-1 w-4/5  "
                  >
                    <MenuItem className="w-full bg-white">
                      <div className=" ">
                        <>
                          <div className="flex rounded-md  items-center text-center bg-[#F5F5F3]">
                            <div className="invisible w-16 items-center">a</div>
                            <div className="w-56 border ">Tên sản phẩm</div>
                            <div className="w-fit  border ">Màu</div>
                            <div className="w-fit  border ">Kích cỡ</div>
                            <div className="w-fit  border ">Số lượng</div>
                            <div className="w-fit  border ">Giá</div>
                          </div>

                          {item.order_details?.map((item) => (
                            <div
                              id={item.id_order}
                              className="w-min flex justify-between rounded-md items-center text-center border-b px-4 py-2	"
                            >
                              <img
                                ...
                                alt="productImage"
                              />
                              <div className="align-center px- w- border">
                                {item.product.name_product}
                              </div>
                              <div className="px- w- border">
                                {item.colors?.map((item) => item.desc_color)}
                              </div>
                              <div className="px- w- border">
                                {item.sizes?.map((item) => item.desc_size)}
                              </div>
                              <div className="px- w- border">{item.quantity}</div>
                              <div className="px- w- border">{item.totalprice}</div>
                            </div>
                          ))}
                        </>
                      </div>
