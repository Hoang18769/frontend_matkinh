...
const Feedback = ({ state }) => {
  ...
  let feed=Array.apply({value:5, comment:"abc"},Array(prop.order_details?.length))
      ...
      {prop.order_details?.map((item) => (
        ...
        <div className="border items-center">
          <div className="flex justify-center items-center px-8 py-4">
            ...
            {prop.feedback.find((items)=>items.id_product_variants===item.id_product_variants)?
              <p>bạn đã feedback rồi</p>:<>  <Box
      sx={{
        '& > legend': { mt: 2 },
      }}
    >
      <Rating id={item.id_orderdetail}
              name="simple-controlled"
        value={value.feed[0].value}
        // value=defaultValue
        onChange={( newValue) => {
          // value=newValue;
          //setValue(newValue);
