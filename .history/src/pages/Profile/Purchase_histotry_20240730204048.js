        ...
        <div className="w-4/5 border items-center">
            ...
            {history.map((item)=>(
              <>
                ...
                <div className=" px-8 py-4 border justify-between">
                {item.order_details?.map((item)=>(
                  <div className="flex">
                  <img className="w-12 h-12" src={item.product.avt_product} alt="productImage" />
                  <div className="align-center px">{item.product.name_product}</div>
                  <div className="px-4">{item.colors?.map((item)=>item.desc_color)}</div>
                  </div>
