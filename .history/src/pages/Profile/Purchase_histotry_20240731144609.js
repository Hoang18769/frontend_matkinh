...
const Purchase_history = () => {
  ...
  return (
    <div className="w-full flex px-8 py-8 gap-5">
      ...
      <div className="w-4/5 border items-center pb-8">
        <div className="flex px-8 py-8 border justify-between">
          {list.map((item) => (
            <label className=" px-2 py-4 w-1/4">{item}</label>
          ))}
        </div>
        {history.map((item) => (
          <>
            <div className="flex px-16 py-4 border justify-between">
              <div className=" px-8 py-2">{item.id_order}</div>
              <div className=" px-8 py-2">{item.date_order}</div>
              <div className=" px-8 py-2">{item.total_order}</div>
              <div className=" px-8 py-2">{item.status_order} </div>
             
             
            </div>
            <div className="w-full ">
              <Menu>
                <MenuButton className="block center relative w-[100%] bg-slate-200 rounded-lg"> Xem chi tiết</MenuButton>
                <MenuItems
                  anchor="bottom"
                  className=" px-8 py-4 block center relative w-[60%] rounded-lg justify-between"
                >
                  {item.order_details?.map((item) => (
                    <MenuItem className="block center relative w-[100%] bg-white rounded-lg">
