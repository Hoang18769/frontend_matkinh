  ...
  return (
    <div className="w-full flex px-8 py-8 gap-5">
      ...
      <div className="w-4/5 items-center px-8">
        <div className="flex  py-4  justify-between bg-[#F5F5F3] rounded-md">
          {list.map((item) => (
            <label className="w-52 py-4 border text-center">{item}</label>
          ))}
          <label className="border w-52 py-4 ">hidden</label>
        </div>
        <div className="">
          {history.map((item) => (
            <div className=" rounded-md  border-b  px-8 items-center ">
              <Menu>
                <div className="w-full items-center flex justify-between">
                  <MenuButton className="border w-full">
                    <div className="flex py-4 rounded-md justify-between items-center text-center hover:underline underline-offset-[4px] ">
                      <div className="w-52 px-4 py-2 border">{item.id_order}</div>
                      <div className="w-52 px-4 py-2 border">{item.date_order}</div>
                      <div className="w-52 px-4 py-2 border">{item.total_order}</div>
                      {status_order_List.map((items) =>
                        items.id === item.status_order ? (
                          <>
                            <div className="w-52 px-4 py-2 border">{items.desc} </div>
                          </>
                        ) : null
                      )}
                    </div>
                  ...
                  {item.status_order === 4 ? (
                    <>
                      <div
                        ...
                        className="text-center border w-52 hover:bg-[#F5F5F3]"
                      >
                        Đánh giá
                      </div>
                    </>
                ...
                <div className="w-full">
                  <MenuItems
                    anchor="bottom"
                    className="z-10  w-[80%] px-16 flex-1 w-4/5 mr-20 "
                  >
