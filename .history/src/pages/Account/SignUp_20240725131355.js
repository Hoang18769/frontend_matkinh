...
const SignUp = () => {
  ...
  const handleSignUp = (e) => {
    ...
    if (checked) {
      if (!name_account) {
        setErrname_account("Enter your name");
      }
      if (!email_account) {
        setErremail_account("Enter your email_account");
      } else {
        if (!email_accountValidation(email_account)) {
          setErremail_account("Enter a Valid email_account");
        }
      }
      if (!phone_customer) {
      ...
      if (!password_account) {
        ...
        if (password_account.length < 6) {
          setErrPassword("Passwords phải từ 6 kí tu");
        }
      }
      if (!address_customer) {
      ...
      if (!name_customer) {
        setErrNameCus("Enter your name custom");
      }
      if (!email_customer) {
                ...
                <div className="flex flex-col gap-.5">
                  ...
                  <input
                    ...
                    value={email_account}
                     class="w-full px-3 py-2 placeholder-gray-300 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-100 focus:border-indigo-300"
                    type="email_account"
                    placeholder="dung@workemail_account.com"
                  />
                  {erremail_account && (
                    <p className="text-sm text-red-500 font-titleFont font-semibold px-4">
                      <span className="font-bold italic mr-1">!</span>
