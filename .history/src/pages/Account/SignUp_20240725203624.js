...
const SignUp = () => {
  ...
  const [email_customer, setemail_customer] = useState("");
  ...
  const [erremail_customer, setErremail_customer] = useState("");
  const [errPhone, setErrPhone] = useState("");
  const [errPassword, setErrPassword] = useState("");
  const [errAddress, setErrAddress] = useState("");
  const [errName, setErrNameCus] = useState("");
  const [erremail_customerCus, setErremail_customerCus] = useState("");
  ...
  const handleemail_customer = (e) => {
    setemail_customer(e.target.value);
    setErremail_customer("");
  };
  const handlePhone = (e) => {
  ...
  // ================= email_customer Validation start here =============
  const email_customerValidation = (email_customer) => {
    return String(email_customer)
      .toLowerCase()
      .match(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i);
  };
  // ================= email_customer Validation End here ===============
  ...
  const handleSignUp = (e) => {
    ...
    if (checked) {
      ...
      if (!email_customer) {
        setErremail_customer("Vui lòng nhập email tài khoản");
      } else {
        if (!email_customerValidation(email_customer)) {
          setErremail_customer("Vui lòng nhập email tài khoản đã xác thực");
        }
      }
      if (!phone_customer) {
      ...
      if (!name_customer) {
        setErrNameCus("Vui lòng nhập tên khách hàng");
      }
      ...
      if (
        name_account &&
        email_customer &&
        email_customerValidation(email_customer) &&
        password_account &&
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex items-start mdl:items-center gap-2">
                  ...
                  <p className="text-sm text-primeColor">
                    Tôi đồng ý với các chính sách của Mat Kinh Cao Lo{" "}
                    <span className="text-blue-500">Terms of Service </span>và{" "}
